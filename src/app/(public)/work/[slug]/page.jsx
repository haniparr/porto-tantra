    ...
    if (projectItem) {
      ...
      if (
        ...
        sections = attrs.sections.map((section, index) => {
          ...
          if (section.images) {
            // Case 1: Nested Strapi format with .data.attributes.url
            if (section.images.data && Array.isArray(section.images.data)) {
              sectionImages = section.images.data
                .map((img) => getStrapiMedia(img))
                .filter((url) => url !== null);
            }
            // Case 2: Direct array of image URLs (Prisma format)
            else if (Array.isArray(section.images)) {
              sectionImages = section.images.filter((url) => url && url.trim());
            }
          }
