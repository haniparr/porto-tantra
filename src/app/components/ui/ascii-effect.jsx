...
float getChar(float brightness, vec2 p, int style) {
  ...
  else if (style == 1) { // Dense - More filled, blocky patterns (Effecto Dense)
    float threshold = brightness;
    // Create a dithered/dense pattern based on brightness
    float pattern = fract(sin(dot(grid, vec2(12.9898, 78.233))) * 43758.5453);
    if (brightness < 0.1) val = 0.0;
    else if (brightness < 0.3) val = (pattern < brightness * 2.0) ? 0.8 : 0.0;
    else if (brightness < 0.5) val = (pattern < brightness * 1.5) ? 1.0 : 0.2;
    else if (brightness < 0.7) val = (pattern < brightness) ? 1.0 : 0.5;
    else if (brightness < 0.9) val = (pattern < brightness * 0.5) ? 0.7 : 1.0;
    else val = 1.0;
  }
  else if (style == 2) { // Minimal - Simple dots
    float dist = length(p - 0.5) * 2.0;
    val = brightness > 0.5 ? (1.0 - smoothstep(0.0, brightness, dist)) : 0.0;
  }
  else if (style == 3) { // Blocks - Solid squares based on brightness
    val = brightness;
  }
